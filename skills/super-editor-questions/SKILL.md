...
5. 用户要求把题目排版进课件时，加载 `super-editor-book-authoring`。每个 `editor_plan_question_lesson` 传 1..50 个 GUID；简单题组用 summary，复杂题组才用 deep；讲解与练习需要不同 layout 时分别规划。
...
- `scope=book` 的题目计划只表示显式 GUID 的整书编排意图，不自动扫描整本书。需要整书上下文时由 `super-editor-book-authoring` 单独读取 manifest。
